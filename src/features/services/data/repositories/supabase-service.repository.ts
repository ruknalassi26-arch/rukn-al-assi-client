...
import { ServiceEntity, PaginatedServicesEntity } from "../../domain/entities/service.entity";
...
export class SupabaseServiceRepository implements IServiceRepository {
  ...
  async getServices({
    ...
    try {
      ...
      if (error) {
        console.error("[SupabaseServiceRepository] Error querying services:", error.message);
        return { items: [], total: 0, page, pageSize, totalPages: 0 };
      }

      const rawItems = (data || []) as ServiceRowDto[];
